...
class CodeBERTEmbedder:
    ...
    def embed_batch(self, list_texts, max_length=512):
        ...
        """
        encodings = self.tokenizer(list_texts, return_tensors="pt", padding=True, truncation=True, max_length=max_length)
        encodings = {k: v.to(self.device) for k, v in encodings.items()}
        with torch.no_grad():
            outputs = self.model(**encodings)
            hidden_states = outputs.last_hidden_state
            # Moyenne pondérée par le masque d'attention
            attention_mask = encodings["attention_mask"].unsqueeze(-1).expand(hidden_states.size())
