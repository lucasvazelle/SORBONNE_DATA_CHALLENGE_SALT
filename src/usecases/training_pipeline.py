...
class TrainingPipeline:
    ...
    def __init__(
        self, train_csv, train_graph_dir, embedding_dir, output_dir, sample_size=None
    ):
            ...
            if fname.startswith("embedding_"):
                try:
                    data = self.io_manager.load_torch(
                        os.path.join(self.embedding_dir, fname)
                    )
    ...
    def run(self):
        ...
        if self.sample_size:
            sampled_metadata = train_metadata[
                train_metadata["name"].isin(list(embedded_ids))
        ...
        texts, stats = self.feature_extractor.process_graphs(
            self.train_graph_dir, selected_ids
        )
